...
DOMAIN="rt.wangyanming.xyz"
CERT_NAME="wangyanming.xyz"
...
# HTTP -> HTTPS（主域名及所有二级域名）
server {
    listen 80;
    listen [::]:80;
    server_name wangyanming.xyz *.wangyanming.xyz;
...
server {
    ...
    listen [::]:443 ssl http2;
    server_name ${DOMAIN} *.wangyanming.xyz wangyanming.xyz;
    ...
    location = /passenger.html {
        return 301 /passenger;
    }
    ...
    location ~* /(deployment|secrets|backups|logs)/ {
        deny all;
    }
